    ...
    <section id="questions">
        <div class="container">
            ...
            <form role="form" action="@if(isset($client) and isset($sales)) ../apply/{{ $client->id }} @else {{action('ClientController@create')}} @endif" method="post">
                ...
                <input type="number" id="sales_percentage" name="sales_percentage" step="0.1" value="@if(isset($client)){{$client->discount_percentage}}@endif">
                <br>
                <label for="credit_worthy">Credit worthy:</label>
                <select name="credit_worthy" id="credit_worthy* " required value="@if(isset($client)){{$client->credit_worthy}}@endif">
